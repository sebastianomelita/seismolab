...
ESP8266& wifintp= MYWIFI::getWifi();
...
{  
  ...
  // Send an NTP request
  if (!(beginPacket((uint8_t*)timeServer, 123) // 123 is the NTP port
   && write((uint8_t *)&ntpFirstFourBytes)
   && endPacket())) {
  ...
  unsigned long time = read();  // NTP time
  for (byte i = 1; i < 4; i++)
    time = time << 8 | read();
...
int beginPacket(uint8_t* host, uint32_t port){  
  ...
  for(int i=0; i<strlen((char*)host);i++)
     str+=host[i];
  str+="\n";
  if (wifintp.registerUDP(str, port)) {
...
int endPacket(){
  //wifintp= MYWIFI::getWifi();  
  if (wifintp.unregisterUDP()) {
        Serial.print("unregister ntp udp ok\r\n");
		return 1;
    } else {
        Serial.print("unregister ntp udp err\r\n");
		return 0;
    }
}
...
int write(uint8_t *buf){
 //wifintp= MYWIFI::getWifi();
 return wifintp.send((uint8_t *)buf, strlen((char*)buf));
}

uint32_t parsePacket(){
	//wifintpntp= MYWIFI::getWifi();	
	buffer1[0] ='\0';
	return wifintp.recv(buffer1, sizeof(buffer1), 10000);
	last1=0;
}

uint8_t read(){
if(last1<sizeof(buffer1))
   return buffer1[last1++];
else
   return buffer1[last1];
}
...
//strcpy(buf,buffer1);
memcpy((uint8_t *)buf, buffer1, sizeof(buffer1));
return sizeof(buffer1);
}
